  ...
  const TaskCard = ({ task, index, statusStyles }) => {
    ...
    return (
      <motion.div
        ...
        className="task-card group hover:scale-[1.01]"
      ...
      >
        ...
        <p className="text-gray-700 dark:text-gray-300 text-sm line-clamp-2 mb-2.5">{task.description}</p>
    ...
    <div className="card">
      <div className="card-header">
        ...
        <button
          ...
          className="btn btn-primary flex items-center gap-1.5 fixed bottom-4 right-4 z-10 md:static md:z-0 shadow-lg md:shadow-none"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {taskStatuses.map(status => (
            <div key={status.name} className="flex flex-col">
              <div className={`flex items-center gap-2 ${status.color} font-medium mb-2 px-1`}>
                <status.icon size={18} />
              ...
              <Droppable droppableId={status.name}>
                {(provided) => (
                  <div
                    ...
                    className="status-column flex-1"
                  >
                    <AnimatePresence>
                      {isLoading ? (
